...
export async function create(value: any): Promise<void> {
  ...
  if (value instanceof Ingredient) {
    ...
    sql =
      ...
      " values (" +
      "?,".repeat(arg.length).substring(0, arg.length * 2 - 1) +
      ");";
...
async function read(
  ...
  var sql: string =
    "select * from " + schema.name + " where " + property + " = ?;";
    ...
    default:
      if (value instanceof Array) {
        const rows = await readAll(
          HistorySchema,
          "date",
          (value[0] as number).toString().padStart(4, "0") +
            "-" +
            (value[1] as number).toString().padStart(2, "0"),
          true
        );
        const Historys: History[] = [];
        for (const row of rows) {
...
export async function deleteFile(verbose = false) {
  const dir: string[] = await FileSystem.readDirectoryAsync(
    FileSystem.documentDirectory + "/SQLite/"
  );
  if (dir.indexOf("DB.db") != -1) {
    for (const file of dir) {
      await FileSystem.deleteAsync(
        FileSystem.documentDirectory + "/SQLite/" + file
      );
    }
    if (verbose) {
      if (
...
async function createCSVFile(verbose = false): Promise<string> {
  ...
  for (const ing of ings) {
    txt =
      ...
      "," +
      (ing.expiryDate == undefined
        ? "undefined"
        : ing.expiryDate?.toLocaleDateString()) +
      "," +
      (ing.useDate == undefined
        ? "undefined"
